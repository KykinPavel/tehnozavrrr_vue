  ...
  <aside class="filter">
    ...
    <form class="filter__form form" action="#" method="get" @submit.prevent="submit">
      ...
      <fieldset class="form__block">
        ...
        <Preloader class="custom-preloader" v-if="colorsLoading"/>
        <div v-else-if="!colorsData">Не удалось загрузить цвета
          <button @click.prevent="loadColors">Попробовать ещё раз</button>
        </div>
        <ul class="colors" v-else>
  ...
  export default {
    ...
    computed: {
      ...
      colors() {
        ...
        ? this.colorsData.items.splice(0, 9) : [];// .splice(0, 9) убираем последний цвет(черный) из массива, т.к. он уже есть в массиве
      },
    },
    watch: {
      priceFrom(value) {
