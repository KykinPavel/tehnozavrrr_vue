        ...
        <div class="item__form">
          <form class="form" action="#" method="POST" @submit.prevent="addToCart">
            ...
            <div class="item__row">
              ...
              <button class="button button--primery" type="#" :disabled="productAddSending">
                В корзину
              </button>
            </div>
            <div v-show="productAdded">Товар добавлен в корзину</div>
            <div v-show="productAddSending">Добавляем товар корзину...</div>
          </form>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      amount: {
        get() {
            const cartProducts = this.$store.state.cartProducts.find(
              item => item.productId === +this.$route.params.id)
            if (cartProducts) {
                return cartProducts.amount;
            }
            return 1;
          },
        set(value) {
